...
class DatasetLoader(Dataset):
    ...
    def news_env_init(self, tmp_data_file):
        ...
        for i, macro_env_pairs in enumerate(tqdm(post_env_pairs)):
            ...
            if len(macro_env_pairs) == 0 or len(news_vecs[macro_env_idxs]) < micro_env_num:
                print('\nPost {} 没有外部新闻!\n'.format(i))
            ...
            try:
                temp = cos_func(avg_micro_vec.repeat(micro_env_num, 1), micro_env_vecs).tolist()
                self.SimDict['avgmic-mic'][i] = temp
            except:
